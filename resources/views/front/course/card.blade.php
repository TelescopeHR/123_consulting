<div class="course-card-items">
		...
		@endphp
	<h3 class="course-card-items-title" title="{{ $course->title }}">
		<a href="{{ route('courses.details', ['slug' => $course->slug_relation->slug]) }}"
			title="{{ $course->title }}">{{ ucwords($course->title) }}</a>
	</h3>
	<div class="courseitem-excerpt course-card-items-desc">
		{!! !empty($description) ? $description[0] . (isset($description[1]) ? '. ' . $description[1] : '') . '. ' : '' !!}
	</div>
	@php
			$average_ratings = 0;
		...
		<div class="courseitem-ratings">
			<div class=" d-flex align-items-center mt-0">
				...
				<div class="mx-2 course-ratings-box" data-average-ratings="{{ number_format($average_ratings, 2) }}"></div>
				<p class="course-card-total-reviews mb-0">{{ $course->reviews->count() }} Reviews</p>
			</div>
		</div>
		@endunless
	...
	<div class="courseitem-actions course-card-action d-flex justify-content-between align-items-center">
		...
		<span class="course-card-price">
			${{ floor($course->price) != $course->price ? number_format($course->price, 1) : number_format($course->price, false) }}
		</span>
		@php
			$is_in_progress = check_course_in_progress($course->id);
		@endphp
		@if ($is_in_progress)
			<a class="bttn bttn-secondary" href="{{ route('user.course', [$is_in_progress->id]) }}" title="Go to Course">
				Go to Course
			</a>
		@else
			...
			@else
			<div class="metabox-add-to-cart-{{$course->id}}">
				<a class="bttn bttn-primary bttn-cart btn-add-cart  card-addto-cart-btn button-primary"
					href="{{ route('cart.add', $course->id) }}" title="Add to Cart" data-id="{{$course->id}}">
