...
@section('meta_description', $course->seo_description)

@if (isset($course) && !empty($course->image) && file_exists(public_path('images/course/' . $course->image)))
	@section('meta_image', asset('images/course/' . $course->image))
@endif
...
@section('content')
		...
		<div class="container">
			<div class="course-inner-container">
				<div class="banner-heading-content text-start">
					<h1 class="course-details-content-title section_heading"> {{ $course->title }} </h1>
					<p class="course-details-content-dis"> {!! $course->description !!} </p>
					<div class="course-details-lessons d-flex justify-content-end align-items-center">
						<div class="course-details-lessons-box d-flex align-items-center">
							<img src="{{ asset('assets/images/bookmark-icon.svg') }}" width="20px" height="20px" alt="bookmark icon">
							@if ($course->lessons)
								<span class="course-details-lessons-value"> {{ $course->lessons->count() }} </span> lessons
							@endif
						</div>
					</div>
				</div>
			...
			<div class="course-inner-container">
				<div class="course-details-pricing-box">
					<div class="course-details-pricing-items course-details-pricing-status">
						<div class="pricing-items-icon-with-text d-flex align-items-center">
							<img src="{{ asset('assets/images/status-icon.svg') }}" width="24px" height="24px" alt="status icon">
							Status
						</div>
						<div class="pricing-items-notenrolled-tag">
							@if (check_course_already_purchased($course->id))
								Enrolled
							@else
								Not Enrolled
							@endif
					...
					<div class="course-details-pricing-items course-details-pricing-price">
						<div class="pricing-items-icon-with-text d-flex align-items-center">
							<img src="{{ asset('assets/images/dollar-icon.svg') }}" width="22px" height="22px" alt="dollar icon">
							Price
						</div>
						<div class="pricing-items-price-tag"> ${{ number_format($course->price, 1) }} </div>
					</div>
					<div class="course-details-pricing-items course-details-pricing-cart justify-content-center">
						<div class="metabox-addtocart">
							@if (check_course_in_cart($course->id))
								<a
									class="btn button-primary px-5 card-addto-cart-btn course-details-pricing-addtocart w-100 justify-content-center"
									href="{{ route('cart') }}" title="Go to Cart">
									Go to Cart
								</a>
							@else
								<a href="{{ route('cart.add', $course->id) }}" class="btn button-primary card-addto-cart-btn course-details-pricing-addtocart w-100 justify-content-center btn-add-cart">
				...
				<div class="course-details-course-curriculum">
					...
					@if ($course->categories)
						@unless ($course->categories->contains('id', 1))
							<div class="course-curriculum-heading d-flex align-items-center justify-content-between">
								...
								<div class="course-curriculum-video-quiz d-flex align-items-center">
									<img src="{{ asset('assets/images/video-play-icon.svg') }}" width="24px" height="24px" alt="video icon"
										class="video-play-icon">
									<div class="curriculum-video-quiz-title">
										@if ($course->lessons)
											<span class="curriculum-video-quiz-value"> {{ $course->lessons->count() }} </span>
										@endif Videos
									</div>
									<div class="curriculum-video-quiz-title curriculum-quiz-box">
										@if ($course->quizzes)
											<span class="curriculum-video-quiz-value"> {{ $course->quizzes->count() }}
											</span>
										@endif Quiz
									</div>
								</div>
					...
					<div class="course-curriculum-lesson">
						<div class="accordion" id="faqAccordion">
							@foreach ($lessons as $key => $lesson)
								<div class="accordion-item">
									<h2 class="accordion-header" id="CurriculumHeading{{ $key }}">
										<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
											data-bs-target="#CurriculumLesson{{ $key }}" aria-expanded="false"
											aria-controls="CurriculumLesson{{ $key }}">
											<img src="{{ asset('assets/images/check-circle-fill-icon.svg') }}" width="24px" height="24px"
												alt="check icon">
											{{ $lesson->title }}
										</button>
									</h2>
									<div id="CurriculumLesson{{ $key }}" class="accordion-collapse collapse"
										aria-labelledby="CurriculumHeading{{ $key }}" data-bs-parent="#faqAccordion">
						...
						<div class="d-flex">
							<div class=" mx-auto">
								{{ $lessons->links('components.custom-pagination') }}
							</div>
						</div>
					</div>
	...
	@if ($related_courses && count($related_courses))
								...
								<div class="tabing-content-details ">
									<div class="course-card-box">
										@foreach ($related_courses as $related_course)
											@include('front.course.card', ['course' => $related_course])
										@endforeach
									</div>
								</div>
							</div>
