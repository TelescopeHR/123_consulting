			...
			<div class="coupon-code-left">
				<div class="alert alert-dismissible fade show p-0 m-0 d-flex align-items-center coupon-code-title" role="alert">
					...
					<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
						<img src="{{ asset('assets/images/close-btn-icon.svg') }}" width="24px" height="24px" alt="close icon">
					</button>
				</div>
			</div>
			...
			<div class="row flex-nowrap">
				<nav class="navbar navbar-expand-lg justify-content-between align-items-center py-0">
					<a class="navbar-brand p-0 m-0" href="{{ route('front.home') }}">
						<img src="{{ asset('assets/images/logo.svg') }}" alt="logo" width="170" height="35" />
					</a>
					...
					<div class="collapse navbar-collapse navigation-barmenu" id="navbarSupportedContent">
						<ul class="navbar-nav">
							<li class="nav-item">
								<a class="nav-link" href="{{ route('front.how-it-works') }}">Agency Startups</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" href="{{ route('front.page', ['policies']) }}">Policy Manuals</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" href="{{ route('front.consultation-booking') }}">Consultation Booking</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" href="{{ route('front.page', ['texas-education']) }}" title="Admin Classes">Admin
									Classes</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" href="{{ route('intake.form') }}">Intake Form</a>
							</li>
							<li class="nav-item dropdown">
								<a class="nav-link dropdown-toggle" href="/" id="navbarDropdown" role="button" data-bs-toggle="dropdown"
									aria-expanded="false">
									Resources
									<img src="{{ asset('assets/images/arrow-down-icon.svg') }}" width="24px" height="24px" alt="">
								</a>
								<ul class="dropdown-menu" aria-labelledby="navbarDropdown">
									<li>
										<a title="Blog" href="{{ route('front.resources') }}" class="dropdown-item">Blog</a>
									</li>
									<li>
										<a title="How-to Videos" href="{{ route('front.video') }}" class="dropdown-item">How-to Videos</a>
									</li>
								</ul>
							</li>
						</ul>
						<div class="header-button-box d-flex align-items-center">
							...
							@endphp
							<a href="{{ route('cart') }}" class="btn header-cart-btn d-flex align-items-center p-0">
								<img src="{{ asset('assets/images/shopping-cart-icon.svg') }}" width="24px" height="24px" alt="cart icon">

								<span id="cart-item" class="cart-item-count">{{ $cart_count ? "($cart_count)" : '' }}</span>
							</a>
							@if (Auth::check())
								<a href="{{ route('logout') }}" class="btn header-login-btn d-flex align-items-center p-0" title="Login"
									onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Logout</a>
								<a href="{{ route('admin.dashboard') }}" class="btn button-primary header-sign-up-btn"
									title="Dashboard">Dashboard</a>
							@else
								<a href="{{ route('login') }}" class="btn header-login-btn d-flex align-items-center p-0">
									Log In
								</a>
